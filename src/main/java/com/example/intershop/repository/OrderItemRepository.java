...
public interface OrderItemRepository extends ReactiveCrudRepository<OrderItem, Long> {
    ...
    Flux<ItemUi> findByTitleLikeOrDescriptionLike(Long orderId, String title, String description, Pageable pageable);
    ...
    Flux<ItemUi> findAll(Long orderId, Pageable pageable);

    Mono<Long> countByOrderId(Long orderId);

    @Query("select count(*) from items i join order_items oi on oi.item_id = i.id and oi.order_id = :orderId where (i.title like :title or i.description like :description)")
